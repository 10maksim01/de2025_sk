...
# Запуск:               sh='PVE-ASDaC-BASH.sh';curl -sOLH 'Cache-Control: no-cache' "https://raw.githubusercontent.com/10maksim01/de2025_sk/main/$sh"&&chmod +x $sh&&./$sh;rm -f $sh
...
function deploy_stand_config() {
    ...
    for elem in $(printf '%s\n' "${!config_var[@]}" | grep -P '^[^_]' | sort); do
        ...
        for opt in $(printf '%s\n' "${!vm_config[@]}" | sort); do
            case "$opt" in
                startup|tags|ostype|serial0|serial1|serial2|serial3|agent|scsihw|cpu|iso_0|cores|cpulimit|cpuunits|memory|bios|bwlimit|description|args|arch|vga|kvm|rng0|acpi|tablet|reboot)
